...
public class PanelNodeInfo {
    ...
    PanelNodeInfo(HorizontalLayout infoTab) {
        // INFO TAB
        //infoTab.setSizeFull();
    	infoTab.setWidth(Sizeable.SIZE_UNDEFINED, 0); // Default
    	infoTab.setHeight(Sizeable.SIZE_UNDEFINED, 0); // Default
	...
	public boolean refresh(NodeInfo nodeInfo) {
		...
		String values[] = {
			...
			(value = nodeInfo.getPublicIP()) != null ? value : NOT_AVAILABLE,
			(value = nodeInfo.getPrivateIP()) != null ? value : NOT_AVAILABLE,
		...
		for (MonitorRecord monitor : monitorsList) {
			...
			MonitorData newMonitorData = new MonitorData(monitorID, systemID, nodeID, chartTime, chartInterval, chartPoints);
